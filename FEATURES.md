...
| ultieconomy.persistence.primary-wallet-merge | Once, when the module loads — first thing in `registerSelf`, so before its Vault provider and PlaceholderAPI expansion are registered, and before the framework registers its commands, listeners and scheduled tasks (all of which follow a successful `registerSelf`): every `currency_balances` row of the primary currency (the second wallet UltiEconomy 2.0.0 kept for it) is added to that player's account — cash to cash, bank to bank, amounts below zero not taken — and removed; a player with only such rows gets an account holding exactly their amounts above zero — an account holding 0 when they hold nothing above zero, so a first join afterwards does not grant `initial-cash` again. One INFO line per merged player (`Primary-currency wallet merge: added <cash> cash and <bank> bank from <player>'s second wallet to their account, which now holds <cash> cash and <bank> bank.` under `language: en`) and one total line; a WARN line for a negative amount, for a player whose merged balance cannot be held exactly — amounts are added as the decimals they print as (0.1 + 0.2 is 0.3), and a sum that overflows, has more significant digits than a balance keeps, or is not a number is not merged (nothing moves, the row stays) — and for a player whose earlier, interrupted merge no longer adds up, or whose rows only have a marker's shape without holding what it records (their rows are left for an operator). Each player's merge marks the rows first, credits the account with absolute values second and removes the rows last, each step durable before the next (the JSON backend is flushed between steps), and only marked rows are ever removed (a start that finds a player's marking interrupted finishes it first), so a start interrupted at any point is completed by the next without adding anything twice (a JSON file torn by a stop mid-rewrite excepted); a start that finds nothing to merge logs nothing. A storage failure logs an ERROR and refuses the module (maintainer decision 2026-09-24, confirmed 2026-09-25; UltiKits/UltiEconomy#25). Servers sharing one database run it one at a time: before merging, a server inserts a claim row with a fixed id into `economy_wallet_merge_claim` (the table's primary key admits one; the table is created only when something is left to merge), changes its heartbeat every 5 seconds at most while merging, and deletes its own claim when done; a server that finds another server's claim logs `Primary-currency wallet merge: another server sharing this database is running the merge (it took the claim at <time>). This server waits for it to finish before UltiEconomy starts.` every 10 seconds and waits, reading only the claim row — during startup, before the server watchdog is armed — until the claim is free, then takes it (one INFO line) and runs the merge, which after a finished merge finds nothing and writes nothing; a claim whose holder and heartbeat have not changed for 30 seconds, by the waiting server's own clock, is taken over with one WARN line and its half-done merge finished without adding anything twice; a holder checks the claim is still its own at every heartbeat and before every write to an account, and one that finds it taken over stops merging (ERROR, module refused); a claim row with no holder or no heartbeat, which the module never writes, refuses the module with an ERROR telling the operator to delete it (maintainer decision 2026-09-25) | persistence | start the server after upgrading from 2.0.0 | n/a | n/a | admin | detailed | PrimaryWalletMerge#run, MergeClaim#runExclusively |
